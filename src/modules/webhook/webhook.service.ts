...
export class WebhookService {
  ...
  private webhookUrls: string[] = [
    ...
    'https://api.example.com/webhooks/medical-processing'
  ];
  ...
  constructor() {
    this.logger.log(`Webhook service initialized with ${this.webhookUrls.length} URLs`);
  ...
  async fireWebhooks(payload: WebhookPayload): Promise<void> {
    if (this.webhookUrls.length === 0) {
      this.logger.debug('No webhook URLs configured, skipping webhook notifications');
      return;
    }

    this.logger.log(`Firing webhooks for job ${payload.jobId} with status: ${payload.status}`);

    // Fire all webhooks concurrently
    const webhookPromises = this.webhookUrls.map(url => this.sendWebhook(url, payload));
    ...
    try {
      ...
      this.stats.successRate = (this.stats.successfulDeliveries / this.stats.totalAttempts) * 100;
      this.stats.lastDeliveryAt = new Date().toISOString();
      
      this.logger.log(`Webhook delivery summary for job ${payload.jobId}: ${successful} successful, ${failed} failed`);
    } catch (error) {
      this.logger.error(`Error in webhook delivery for job ${payload.jobId}:`, error);
    }
  }
  ...
  private async sendWebhook(url: string, payload: WebhookPayload): Promise<void> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.logger.log(`✅ Webhook delivered to ${url} for job ${payload.jobId} (${response.status})`);
      } else {
        this.stats.urlStats[url].failures++;
        this.logger.error(`❌ Webhook failed to ${url} for job ${payload.jobId}: HTTP ${response.status}`);
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
    } catch (error) {
      this.stats.urlStats[url].failures++;
      this.logger.error(`❌ Failed to deliver webhook to ${url} for job ${payload.jobId}:`, error.message);
      throw error;
    }
  }
